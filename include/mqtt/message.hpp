...
class header_packet_id_32_message {
public:
    /**
     * @brief Create empty header_packet_id_message.
     */
    header_packet_id_32_message(std::uint8_t type, std::uint8_t flags, std::uint32_t packet_id)
        : message_ { make_fixed_header(type, flags), 4, MQTT_32BITNUM_TO_BYTE_SEQ(packet_id) }
    {}

    template <typename Iterator>
    header_packet_id_32_message(Iterator b, Iterator e) {
...
struct puback_32_message : detail::header_packet_id_32_message {
    puback_32_message(std::uint32_t packet_id)
        : detail::header_packet_id_32_message(control_packet_type::puback, 0b0000, packet_id)
    {}
};

struct pubrec_32_message : detail::header_packet_id_32_message {
    pubrec_32_message(std::uint32_t packet_id)
        : detail::header_packet_id_32_message(control_packet_type::pubrec, 0b0000, packet_id)
    {}
};

struct pubrel_32_message : detail::header_packet_id_32_message {
    pubrel_32_message(std::uint32_t packet_id)
        : detail::header_packet_id_32_message(control_packet_type::pubrel, 0b0010, packet_id)
    {
    }

    template <typename Iterator>
    pubrel_32_message(Iterator b, Iterator e)
        : header_packet_id_32_message(b, e)
    {
    }
     ...
     * @return packet_id
     */
    std::uint32_t packet_id() const {
        return make_uint32_t(message().begin() + 2, message().end());
    }
};

struct pubcomp_32_message : detail::header_packet_id_32_message {
    pubcomp_32_message(std::uint32_t packet_id)
        : detail::header_packet_id_32_message(control_packet_type::pubcomp, 0b0000, packet_id)
    {}
};

struct unsuback_32_message : detail::header_packet_id_32_message {
    unsuback_32_message(std::uint32_t packet_id)
        : detail::header_packet_id_32_message(control_packet_type::unsuback, 0b0000, packet_id)
    {}
};
...
public:
          ...
          remaining_length_(publish_remaining_length(topic_name, qos, payload))
    {
        ...
        if (qos == qos::at_least_once ||
            qos == qos::exactly_once) {
            packet_id_.reserve(4);
            add_uint32_t_to_buf(packet_id_, packet_id);
        }
    }

    template <typename Iterator>
    publish_32_message(Iterator b, Iterator e) {
        if (b >= e) throw remaining_length_error();
        fixed_header_ = *b;
        auto qos = publish::get_qos(fixed_header_);
        ++b;
        ...
        case qos::exactly_once:
            if (b + 4 >= e) throw remaining_length_error();
            std::copy(b, b + 4, std::back_inserter(packet_id_));
            b += 4;
            break;
        default:
            throw protocol_error();
     ...
     */
    std::uint32_t packet_id() const {
        return make_uint32_t(packet_id_.begin(), packet_id_.end());
    }
...
private:
    ...
    as::const_buffer topic_name_;
    boost::container::static_vector<char, 2> topic_name_length_buf_;
    boost::container::static_vector<char, 4> packet_id_;
    as::const_buffer payload_;
    std::size_t remaining_length_;
    boost::container::static_vector<char, 4> remaining_length_buf_;
};

class subscribe_32_message {
private:
    struct entry {
        entry(as::const_buffer const& topic_name, std::uint8_t qos)
            : topic_name(topic_name),
...
public:
    subscribe_32_message(
        ...
        : fixed_header_(static_cast<char>(make_fixed_header(control_packet_type::subscribe, 0b0010))),
          packet_id_ { MQTT_32BITNUM_TO_BYTE_SEQ(packet_id) },
          remaining_length_(4)
    {
        for (auto const& e : params) {
            auto const& topic_name = std::get<0>(e);
...
public:
    suback_32_message(
        ...
        : fixed_header_(static_cast<char>(make_fixed_header(control_packet_type::suback, 0b0000))),
          packet_id_ { MQTT_32BITNUM_TO_BYTE_SEQ(packet_id) },
          remaining_length_(params.size() + 4)
    {
        auto rb = remaining_bytes(remaining_length_);
        for (auto e : rb) {
            remaining_length_buf_.push_back(e);
